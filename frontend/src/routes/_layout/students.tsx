    ...
    <div className="flex gap-3 overflow-x-auto pb-4 no-scrollbar mb-6">
      {filters.map((filter) => (
        <button
          ...
          className={`px-6 py-2 rounded-full font-label text-sm font-semibold whitespace-nowrap transition-colors ${
            activeFilter === filter
...
function StudentCard({ student }: { student: Student }) {
  ...
  return (
    <div
      className={`bg-surface-container-lowest p-6 rounded-xl ambient-shadow flex gap-5 items-start ${
    ...
    >
      <div className="relative">
        ...
        <div
          className={`absolute -bottom-1 -right-1 w-4 h-4 rounded-full border-2 border-surface-container-lowest ${getStatusColor(
            student.status
          )}`}
        />
      </div>
...
// Add Student FAB Component
function AddStudentFAB() {
  return (
    <button className="fixed bottom-28 right-6 w-14 h-14 btn-gradient text-on-primary rounded-xl shadow-lg shadow-primary/30 flex items-center justify-center active:scale-90 transition-transform z-40">
